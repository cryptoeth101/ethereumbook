...
이더리움은 주로 "월드 컴퓨터 (world computer)"로 기술된다. 이것은 무엇을 의미하는 것일까? 컴퓨터 과학에 기반한 설명으로 시작하여, 이더리움을 비트코인 및 다른 탈중앙화 정보 교환 플랫폼 (또는 간단히 "블록체인")과 비교함으로써 이더리움 기능 및 특성을 보다 실질적인 분석으로 설명하고자 한다.
...
컴퓨터 과학 관점에서 이더리움은 결정적 (deterministic)이지만 사실상 무제한 상태 머신 (unbounded state machine)으로 전역적으로 접근가능한 싱글톤 상태 (singleton state) 및 그 상태에 변화를 적용하는 가상 머신으로 구성되어 있다.
...
좀 더 현실적인 관점에서 이더리움은 오픈소스이며  _스마트 컨트랙트 (smart contract)_ 라는 프로그램을 실행하는 전세계적으로 분산된 컴퓨팅 인프라이다. 이더리움은 블록체인을 사용하여 시스템의 상태 변화를 동기화하고 저장하며, _이더 (ether)_ 라는 암호화폐를 사용하여 실행 자원 비용을 측정하고 제한한다.

//The Ethereum platform enables developers to build powerful decentralized applications with built-in economic functions. While providing high availability, auditability, transparency, and neutrality, it also reduces or eliminates censorship and reduces certain counterparty risks.
이더리움 플랫폼은 개발자들이 경제 기능이 내장된 강력한 탈중앙화 어플리케이션을 설계할 수 있도록 해준다. 높은 가용성, 감사 기능성, 투명성 및 중립성을 제공하는 동시에 검열을 줄이거나 제거하고 특정 거래 상대방 위험 (counterparty risk)을 줄일 수 있다.
